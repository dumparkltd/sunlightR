...
get_min_altitude_for_azimuth1 <- function(cellNo, azimuth, dem, resolution) {
  # 
  # 
  ...
  # extract elevation
  elevation = dem[cellNo]
  min_altitude = 0
  
  if (!is.na(elevation)) {
    spt <- as.data.frame(
      spTransform(
        xyFromCell(dem, c(cellNo) , spatial=TRUE),
        CRS("+proj=longlat")
      )
    )
    
    dem_extent <- st_transform(st_as_sfc(st_bbox(dem)), crs=4326)
    transect = get_transect(lon = spt$x, lat = spt$y, azimuth, dem, dem_extent, resolution)
    # plot(transect$elev)
