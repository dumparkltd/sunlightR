...
get_altitude_min_for_azimuth <- function(cellNo, azimuth, dem) {
  ...
  if (!is.na(elevation_origin)) {
    ...
    xy_origin <- xyFromCell(dem, c(cellNo) , spatial=TRUE)
    ll_origin <- as.data.frame(spTransform(xy_origin, CRS("+proj=longlat")))
  
    # calculate transect
    # returns data frame with 2 columns, cellNo and elevations
    transect = get_transect2(lon = ll_origin$x, lat = ll_origin$y, azimuth, dem)
    ...
    for (i in 1:nrow(transect)) {
      ...
      if (!is.na(elevation_target)) {
        ...
        if (d_vert > 0) {
          ...
          xy_target <- xyFromCell(dem, c(transect[i,]$cellNo) , spatial=TRUE)
